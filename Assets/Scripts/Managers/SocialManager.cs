...
public class SocialManager : MonoBehaviour {
  ...
  public void UpdateSocialPanel(string text) {
    ...
    GameObject newText =
        Instantiate(UIManager.MyInstance.usernameObject,
                    UIManager.MyInstance.SocialBoxContent.transform);
  ...
  public void AddAFriend() {
    ...
    if (!string.IsNullOrWhiteSpace(
      ...
      if (true) {
        ...
        chatBoxManager.SendMessageToChat(
            "L'utilisateur a été ajouté à votre liste d'ami.",
            Message.MessageType.server);
      } else {
        // Send an error message
        chatBoxManager.SendMessageToChat("Le destinataire est introuvable.",
                                         Message.MessageType.server);
      }
