    ...
    TowersWebSocket.wsChat.OnMessage += (sender, args) => {
      if (args.Data.Contains("callbackChatMessage")) {
        ...
        try {
          ...
          sendNewMessage = true;
          Debug.Log(callbackChatMessage.callbackChatMessage.sender + " : " +
                    callbackChatMessage.callbackChatMessage.message);
        } catch (Exception e) {
    ...
    if (!string.IsNullOrWhiteSpace(chatBox.text) &&
      ...
      if (chatBox.text.Length > 3 && chatBox.text[0] == '/' &&
        ...
        if (privateMess == true) {
          ...
          TowersWebSocket.ChatSender(
              "ALL", "GENERAL", NetworkingController.NickName, chatBox.text);
        }
      }
      ...
      // Check if it is a invite to a party
        ...
        TowersWebSocket.ChatSender("ALL", "GENERAL",
                                   NetworkingController.NickName, chatBox.text);
      }
