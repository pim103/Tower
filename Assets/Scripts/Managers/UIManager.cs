...
/// <summary>
/// This is the UIManager script, it contains functionality that is specific to
/// the UI
/// </summary>
public class UIManager : MonoBehaviour {
  private static UIManager instance;
  public static UIManager MyInstance{
      get{if(instance == null){instance = FindObjectOfType<UIManager>();
}
...
private void Awake() { tooltipText = tooltip.GetComponentInChildren<Text>(); }

void Update() {
  // Account Management
  goldBarAmountText.text =
      AccountManager.MyInstance.AccountResources[0].Amount.ToString();
  goldNuggetAmountText.text =
      AccountManager.MyInstance.AccountResources[1].Amount.ToString();
  stoneOreAmountText.text =
      AccountManager.MyInstance.AccountResources[2].Amount.ToString();
...
public void ShowTooltip(Resource resource, Vector3 position,
  ...
  Vector3 mouse_position =
      new Vector3(position.x + (tooltipRect.rect.width / 2 + offset_x),
                  position.y - (tooltipRect.rect.height / 2 + offset_y), 0f);
  ...
  tooltipRect.transform.position = new Vector3(
      Mathf.Clamp(mouse_position.x, min.x + tooltipRect.rect.width / 2,
                  max.x - tooltipRect.rect.width / 2),
      Mathf.Clamp(mouse_position.y, min.y + tooltipRect.rect.height / 2,
                  max.y - tooltipRect.rect.height / 2),
