...
/// <summary>
/// This is the GameSettings script, it contains functionality that is specific
...
public class GameSettings : MonoBehaviour {
  ...
  private void Start() {
    // Get volume state
    UIManager.MyInstance.volumeSlider.value =
        PlayerPrefs.GetFloat("GameVolume", 0f);
    ...
    for (int i = 0; i < resolutions.Length; i++) {
      ...
      Screen.height && resolutions[i].refreshRate == Screen.refreshRate)
      {
          currentResolutionIndex = i;
      }*/
    }
    ...
    UIManager.MyInstance.resolutionDropdown.value =
        PlayerPrefs.GetInt("GameResolution", currentResolutionIndex);
    UIManager.MyInstance.resolutionDropdown.RefreshShownValue();

    // Get graphic quality level
    UIManager.MyInstance.graphicQualityLevelDropdown.value = PlayerPrefs.GetInt(
        "GameQualityLevel", QualitySettings.GetQualityLevel());
  }
