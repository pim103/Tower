...
public class DrawIfPropertyDrawer : PropertyDrawer {
  ...
  public override void OnGUI(Rect position, SerializedProperty property,
    ...
    comparedField =
        property.serializedObject.FindProperty(drawIf.comparedPropertyName);
    ...
    try {
      ...
      // This place will only be reached if the type is not a numeric one. If
      // the comparison type is not valid for the compared field type, log an
      // error.
      if (drawIf.comparisonType != ComparisonType.Equals &&
          drawIf.comparisonType != ComparisonType.NotEqual) {
        Debug.LogError("The only comparsion types available to type '" +
                       comparedFieldValue.GetType() +
                       "' are Equals and NotEqual. (On object '" +
