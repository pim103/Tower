...
public class NumericType : IEquatable<NumericType> {
  ...
  public NumericType(object obj) {
    if (!obj.IsNumbericType()) {
      throw new NumericTypeExpectedException(
          "The type of object in the NumericType constructor must be numeric.");
    }
    value = obj;
    type = obj.GetType();
  }
  ...
  public bool Equals(NumericType other) { return this == other; }
  ...
  public static bool operator<(NumericType left, NumericType right) {
    ...
    throw new NumericTypeExpectedException(
        "Please compare valid numeric types.");
  }
