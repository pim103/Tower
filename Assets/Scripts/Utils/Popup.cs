...
namespace Utils
{
    public class Popup : MonoBehaviour
    {
        ...
        [Header("Contextual Parameters")] public bool isContextualDialog; //La popup est prioritaire sur tous les autres ui, et on bloque les input qui ne la concerne pas tant qu'elle n'est pas fermée (en effectuant une certaine action => oui/non, fermer la popup... etc)
        ...
        [Header("Dynamic Parameters")] public bool isDynamic; 
        [DrawIf("isDynamic", true, ComparisonType.Equals, DisablingType.DontDraw)] 
        [SerializeField] private bool isPositionDynamic; //La popup doit se placer dynamiquement à l'écran par rapport à une autre ui (droite/gauche, haut/bas) selon la place dispo
        [DrawIf("isPositionDynamic", true, ComparisonType.Equals, DisablingType.DontDraw)] 
        [SerializeField] private GameObject otherObj;
        [DrawIf("isDynamic", true, ComparisonType.Equals, DisablingType.DontDraw)] 
        [SerializeField] private bool isSizeDynamic; //La popup doit être resize selon le contenu (qui n'est pas déterminé à l'avance) => toutes les popup ne devraient-elles pas avoir cette règle ? 

        [Header("Question Parameters")] public bool isAQuestion; //La popup doit contenir deux boutons (voire trois) à laquelle une action doit être liée
        ...
        [Header("Animation parameters")] public bool isAnimated; //La popup a une petite animation d'apparition (et de disparition ?)
        ...
        private static void Close()
        {
            ...
            if (_this.callbackAdded || _this.isAQuestion && (_this.validateCallbackAdded || _this.cancelCallbackAdded || _this.thirdCallbackAdded))
            {
        ...
        //
        private void InitializeOptionsAttributes() // Traitements à l'initialisation selon les options choisies
        {
