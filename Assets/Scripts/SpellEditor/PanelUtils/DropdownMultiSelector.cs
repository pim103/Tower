...
namespace SpellEditor.PanelUtils {
  ...
  public class DropdownMultiSelector : MonoBehaviour {
    ...
    [SerializeField]
    private SpellComponentPanel spellComponentPanel;
    ...
    private void SelectValue(int newIndex) {
      ...
      if (selectedIndex.Contains(value)) {
        ...
        dropdown.options[newIndex].image =
            Sprite.Create(initialColor, rect, Vector2.zero);
      } else {
        selectedIndex.Add(value);
        dropdown.options[newIndex].image =
            Sprite.Create(selectColor, rect, Vector2.zero);
      }
    ...
    public void InitDropdownMultiSelect() {
      ...
      case SelectedObject.GeometryShader:
        foreach (Material material in spellComponentPanel
                     .geometryShaderMaterials) {
          listNames.Add(material.name);
        }
        break;
      case SelectedObject.ParticleEffect:
        foreach (GameObject particleObject in spellComponentPanel
                     .particleObjects) {
          listNames.Add(particleObject.name);
        }
        break;
      case SelectedObject.AddedMesh:
        foreach (GameObject additionalMesh in spellComponentPanel
                     .additionalMeshs) {
    ...
    public List<int> NamesToIndex() {
      ...
      foreach (var selectedString in selectedIndex) {
        indexList.Add(
            dropdown.options.FindIndex((i) => i.text.Equals(selectedString)));
      }
