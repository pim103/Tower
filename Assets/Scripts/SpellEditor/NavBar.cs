...
public enum Panel { Effect, Spell, SpellComponent, SpellWithCondition }

public class NavBar : MonoBehaviour {
  [SerializeField]
  private SpellPanel spellPanelScript;
  [SerializeField]
  private EffectPanel effectPanelScript;
  [SerializeField]
  private SpellComponentPanel spellComponentPanelScript;
  [SerializeField]
  private Button exportSpells;
  [SerializeField]
  private Button importSpell;

  [SerializeField]
  private Button createSpellButton;
  [SerializeField]
  private Button createSpellComponentButton;
  [SerializeField]
  private Button createSpellEffectButton;
  [SerializeField]
  private Button createSpellWithConditionButton;

  [SerializeField]
  private GameObject spellPanel;
  [SerializeField]
  private GameObject spellComponentPanel;
  [SerializeField]
  private GameObject effectPanel;
  [SerializeField]
  private GameObject spellWithConditionPanel;
  [SerializeField]
  private SpellWithConditionPanel spellWithConditionPanelScript;

  [SerializeField]
  private Button TestSpellButton;
  ...
  private void Start() {
    ...
    createSpellButton.onClick.AddListener(
        delegate { SwitchPanel(Panel.Spell); });
    createSpellComponentButton.onClick.AddListener(
        delegate { SwitchPanel(Panel.SpellComponent); });
    createSpellEffectButton.onClick.AddListener(
        delegate { SwitchPanel(Panel.Effect); });
    createSpellWithConditionButton.onClick.AddListener(
        delegate { SwitchPanel(Panel.SpellWithCondition); });
  ...
  public static void ModifyExistingComponent(object component,
                                             object newComponent) {
  ...
  private void ExportSpells() {
    foreach (KeyValuePair<string, Spell> valuePair in ListCreatedElement
                 .Spell) {
      fsSerializer serializer = new fsSerializer();
      serializer.TrySerialize(valuePair.Value.GetType(), valuePair.Value,
                              out fsData data);
      File.WriteAllText(Application.dataPath + "/Data/SpellsJson/" +
                            valuePair.Key + ".json",
  ...
  private void ChooseSpellToImport() {
    ...
    path = EditorUtility.OpenFilePanel(
        "Choose your spell", Application.dataPath + "/Data/SpellsJson", "json");
#endif
  ...
  private void ParseSpell(Spell spell) {
    if (spell == null ||
        ListCreatedElement.Spell.ContainsKey(spell.nameSpell)) {
      if (spell != null) {
    ...
    ParsePropertyInfo(spell, spell.GetType().GetProperties(
                                 BindingFlags.Public | BindingFlags.NonPublic |
                                 BindingFlags.Instance));
  }
  ...
  private void ParseSpellComponent(SpellComponent spellComponent) {
    if (spellComponent == null ||
        ListCreatedElement.SpellComponents.ContainsKey(
            spellComponent.nameSpellComponent)) {
      if (spellComponent != null) {
        Debug.Log(spellComponent.nameSpellComponent +
                  " already exist (spellComponent)");
      }
      return;
    }

    ListCreatedElement.SpellComponents.Add(spellComponent.nameSpellComponent,
                                           spellComponent);

    ParsePropertyInfo(spellComponent,
                      spellComponent.GetType().GetProperties(
                          BindingFlags.Public | BindingFlags.NonPublic |
                          BindingFlags.Instance));
  }
  ...
  private void ParseSpellWithCondition(SpellWithCondition spellWithCondition) {
    if (spellWithCondition == null ||
        ListCreatedElement.SpellWithCondition.ContainsKey(
            spellWithCondition.nameSpellWithCondition)) {
      if (spellWithCondition != null) {
        Debug.Log(spellWithCondition.nameSpellWithCondition +
                  " already exist (Spell with condition)");
      }
      return;
    }

    ListCreatedElement.SpellWithCondition.Add(
        spellWithCondition.nameSpellWithCondition, spellWithCondition);

    ParsePropertyInfo(spellWithCondition,
                      spellWithCondition.GetType().GetProperties(
                          BindingFlags.Public | BindingFlags.NonPublic |
                          BindingFlags.Instance));
  }
  ...
  private void ParsePropertyInfo(object origin, PropertyInfo[] propertyInfos) {
    foreach (PropertyInfo prop in propertyInfos) {
      if (prop.PropertyType == typeof(Effect) ||
          prop.PropertyType == typeof(List<Effect>)) {
      ...
      } else if (prop.PropertyType == typeof(Spell) ||
                 prop.PropertyType == typeof(List<Spell>)) {
      ...
      } else if (prop.PropertyType == typeof(SpellComponent) ||
                 prop.PropertyType == typeof(List<SpellComponent>)) {
        if (prop.PropertyType == typeof(List<SpellComponent>)) {
          List<SpellComponent> spellComponentsProp =
              (List<SpellComponent>) prop.GetValue(origin);
          foreach (SpellComponent spellComponentIn in spellComponentsProp) {
            ParseSpellComponent(spellComponentIn);
          }
        } else {
          SpellComponent spellComponentProp =
              (SpellComponent) prop.GetValue(origin);
        ...
        if (prop.PropertyType == typeof(List<SpellWithCondition>)) {
          List<SpellWithCondition> spellWithConditionsProp =
              (List<SpellWithCondition>) prop.GetValue(origin);
          foreach (SpellWithCondition spellWithConditionIn in
                       spellWithConditionsProp) {
            ParseSpellWithCondition(spellWithConditionIn);
          }
        } else {
          SpellWithCondition spellWithConditionsProp =
              (SpellWithCondition) prop.GetValue(origin);
  ...
  public void SwitchToTestSpellScene() { SceneManager.LoadScene("TestSpell"); }
}
}
