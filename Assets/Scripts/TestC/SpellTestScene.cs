...
public class SpellTestScene : MonoBehaviour {
  ...
  private void Update() {
    if (Input.GetKeyUp(KeyCode.Keypad0)) {
      SpellController.CastSpell(player.entity, player.entity.spells[3],
                                player.transform.position, player.target);
    }
  ...
  public void BackToEditorAction() { SceneManager.LoadScene("SpellEditor"); }
  ...
  private IEnumerator Waiting() {
    ...
    foreach (KeyValuePair<string, Spell> pair in ListCreatedElement.Spell) {
      ...
      if (countSpells == 0) {
        ...
        extraSpellText[countSpells]
            .transform.GetChild(0)
            .GetComponent<Text>()
            .text = copySpell.nameSpell;
      }
  ...
  private void OtherSpell() {
    ...
    AreaOfEffectSpell area =
        new AreaOfEffectSpell{startPosition = Vector3.zero,
    ...
    Spell newSpell = new Spell{cost = 0, cooldown = 2, castTime = 0,
    ...
    SummonSpell summonSpell = new SummonSpell{
        ...
        attackDamage = 40, attackSpeed = 1, damageType = DamageType.Magical,
        canMove = true, isTargetable = true, idPoolObject = 2, moveSpeed = 10,
        summonNumber = 2, BehaviorType = BehaviorType.Distance,
        positionPresets = positions, isUnique = true, nbUseSpells = 2,
        spells = spells,
        AttackBehaviorType = AttackBehaviorType.AllSpellsIFirst};

    SpellController.CastSpellComponent(
        player.entity, summonSpell, player.transform.position, player.entity);
    //            Effect repulse = new Effect { typeEffect =
    //            TypeEffect.Expulsion, launcher = player.entity, level = 10,
    //            directionExpul = DirectionExpulsion.Out, originExpulsion =
    //            OriginExpulsion.SrcDamage}; List<Effect> effects = new
    //            List<Effect>(); effects.Add(repulse);
  ...
  private void TestSpell() {
    ...
    //                geometry = Geometry.Sphere,
    //                positionToStartSpell =
    //                PositionToStartSpell.DynamicPosition
    //            };

    Effect repulse =
        new Effect{typeEffect = TypeEffect.Expulsion, launcher = player.entity,
                   level = 10, directionExpul = DirectionExpulsion.Out,
                   originExpulsion = OriginExpulsion.SrcDamage};
    List<Effect> effects = new List<Effect>();
    effects.Add(repulse);

    //            Effect regen = new Effect
    //                {launcher = player.entity, level = 2, durationInSeconds =
    //                10, typeEffect = TypeEffect.Regen};
    //
    //            SpellWithCondition spellWithCondition = new SpellWithCondition
    //            { effect = regen, conditionType = ConditionType.IfTargetDies};
    //            List<SpellWithCondition> spellWithConditions = new
    //            List<SpellWithCondition>();
    //            spellWithConditions.Add(spellWithCondition);
    //
    //
    //            Effect resurrect = new Effect
    //                {launcher = player.entity, level = 1, durationInSeconds =
    //                10, typeEffect = TypeEffect.Resurrection};
    ...
    //
    //            player.entity.att = 15;
    //            SpellController.CastSpellComponent(player.entity, buffSpell,
    ...
    //                * 1.5f), effectsOnHit = effects, damages = 10,
    ...
    MovementSpell movementSpell =
        new MovementSpell{duration = 3f,
                          speed = 20,
    ...
    SpellController.CastSpellComponent(player.entity, movementSpell,
                                       player.positionPointed, player.target);
  }
}
}
