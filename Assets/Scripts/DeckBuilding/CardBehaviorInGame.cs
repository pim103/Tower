...
public class CardBehaviorInGame : MonoBehaviour {
  ...
  public void SetCard(int type) {
    ...
    if (type == 0) {
      ...
      rangeSphere.transform.localScale =
          new Vector3(group.radius * 4, group.radius * 4, group.radius * 4);
  ...
  public void InstantiateGroupsMonster(GroupsMonster groups, Vector3 position,
                                       Transform pGroupParent) {
    Monster monster;
    int nbMonsterInit = 0;
    ...
    foreach(KeyValuePair<int, int>mobs in groups.monsterInGroups) {
      for (int i = 0; i < mobs.Value; i++) {
        ...
        GameObject monsterGameObject =
            Instantiate(monster.model, pGroupParent, true);
        Rigidbody monsterRigidBody =
            monsterGameObject.GetComponent<Rigidbody>();
