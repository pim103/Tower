...
/// <summary>
/// This is the Achievment script, it contains functionality that is specific to
/// the Achievment
/// </summary>
public class Achievment {
  ...
  public Achievment(string name, string description, int points,
                    int spriteIndex, GameObject achievmentRef,
                    int maxProgression) {
    this.name = name;
  ...
  public bool EarnAchievment() {
    if (!unlocked && !dependencies.Exists(x => x.unlocked == false) &&
        CheckProgress()) {
      achievmentRef.GetComponent<Image>().color =
          AchievmentManager.Instance.UnlockedColor;
      SaveAchievment(true);
  ...
  public void LoadAchievment() {
    ...
    if (unlocked) {
      AchievmentManager.Instance.textPoints.text =
  ...
  public bool CheckProgress() {
    ...
    if (maxProgression > 0) {
      achievmentRef.transform.GetChild(0).GetComponent<Text>().text =
