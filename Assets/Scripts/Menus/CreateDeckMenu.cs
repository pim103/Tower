...
public class CreateDeckMenu : MonoBehaviour, MenuInterface {
  ...
  [SerializeField]
  private Transform ColPosition;
  [SerializeField]
  private Transform ColPositionXOffset;
  [SerializeField]
  private Transform ColPositionYOffset;
  [SerializeField]
      ...
      foreach (Card card in selectedDeck.GetCardsInDeck()) {
        ...
        currentCardInDeckButton.transform.position =
            new Vector3(deckPosition.position.x,
                        deckPosition.position.y - 30 * currentCount - 1, 0);
  ...
  public void ShowCards() {
    ...
    foreach (Card card in DataObject.CardList.GetCardsInCollection()) {
      ...
      currentCardInCollButton.transform.position = new Vector3(
          ColPosition.position.x +
              (ColPositionXOffset.position.x - ColPosition.position.x) *
                  (currentCount % 5),
          ColPosition.position.y +
              ((ColPositionYOffset.position.y - ColPosition.position.y) *
               (ycount - 1)) +
              20,
          0);
    ...
    if (cardNb > 1) {
      ...
      foreach (GameObject button in cardInDeckButtonsList) {
        button.transform.position =
            new Vector3(deckPosition.position.x,
                        deckPosition.position.y - 30 * posCount - 1,
  ...
  public IEnumerator AddNewDeck(string deckName, int isMonsterDeck) {
    ...
    var www = UnityWebRequest.Post(
        NetworkingController.PublicURL + "/services/game/deck/add.php", form);
  ...
  public IEnumerator DeleteCardsInDeck(int deckId) {
    ...
    form.AddField("gameToken", NetworkingController.GameToken);
    var www =
        UnityWebRequest.Post(NetworkingController.PublicURL +
                                 "/services/game/deck/deleteAllCardsInDeck.php",
                             form);
    www.certificateHandler = new AcceptCertificate();
    yield return www.SendWebRequest();
    yield return new WaitForSeconds(0.5f);
  ...
  public IEnumerator AddNewCardInDeck(int deckId, int cardId, int nbCards) {
    ...
    var www = UnityWebRequest.Post(NetworkingController.PublicURL +
                                       "/services/game/deck/addCardInDeck.php",
                                   form);
    www.certificateHandler = new AcceptCertificate();
    yield return www.SendWebRequest();
