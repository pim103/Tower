...
public class ShopWindow : MonoBehaviour {
  ...
  [SerializeField]
  private bool newShopPage;
  [SerializeField]
  private bool topShopPage;
  [SerializeField]
  private bool ressourceShopPage;

  [Header("Shop Containers List")]
  [SerializeField]
  private ShopContainer[] shopContainers;
  ...
  public void AddItems() {
    ...
    if (pages.Count > 0) {
      for (int i = 0; i < pages[pageIndex].Count; i++) {
        if (pages [pageIndex]
            [i] != null) {
          shopContainers[i].AddItem(pages [pageIndex]
                                    [i]);
        }
      }
    }
