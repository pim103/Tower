...
public class ContentGenerationEditor : EditorWindow {
  private enum Category { WEAPON, ARMOR, SPELL, MONSTER, PLAYER }
  ...
  public void DisplayHeader() {
    ...
    if (GUILayout.Button(EditorConstant.weaponSprite, GUILayout.Width(75),
                         GUILayout.Height(75))) {
      currentCategory = Category.WEAPON;
      currentEditor = weaponEditor;
    } else if (GUILayout.Button(EditorConstant.armorSprite, GUILayout.Width(75),
                                GUILayout.Height(75))) {
      currentCategory = Category.ARMOR;
      currentEditor = armorEditor;
    } else if (GUILayout.Button(EditorConstant.monsterSprite,
                                GUILayout.Width(75), GUILayout.Height(75))) {
      currentCategory = Category.MONSTER;
      currentEditor = monsterEditor;
    } else if (GUILayout.Button(EditorConstant.playerSprite,
                                GUILayout.Width(75), GUILayout.Height(75))) {
      currentCategory = Category.PLAYER;
      currentEditor = playerEditor;
    } else if (GUILayout.Button(EditorConstant.spellSprite, GUILayout.Width(75),
                                GUILayout.Height(75))) {
  ...
  void OnGUI() {
    ...
    // INIT DISPLAY HEADER
    GUILayout.BeginArea(
        new Rect(0, 0, EditorConstant.WIDTH, EditorConstant.EIGHTH_HEIGHT * 2));
    DisplayHeader();
    GUILayout.EndArea();
    // END DISPLAY HEADER

    // INIT FORM DISPLAY
    GUILayout.BeginArea(new Rect(0, EditorConstant.EIGHTH_HEIGHT * 2,
                                 EditorConstant.WIDTH,
                                 EditorConstant.EIGHTH_HEIGHT * 5));
  ...
  private void SaveChanges() {
    if (DataObject.EquipmentList != null) {
      ...
      foreach (Weapon weapon in DataObject.EquipmentList.weapons) {
        if (!Utils.Tools.IsSimilar(weapon,
                                   weaponEditor.originalWeapon[weapon.id])) {
      ...
      foreach (Armor armor in DataObject.EquipmentList.armors) {
        if (!Utils.Tools.IsSimilar(armor,
                                   armorEditor.originalArmor[armor.id])) {
    ...
    if (DataObject.MonsterList != null) {
      ...
      foreach (Monster monster in DataObject.MonsterList.monsterList) {
        if (!Utils.Tools.IsSimilar(monster,
                                   monsterEditor.origMonsterList[monster.id])) {
      ...
      foreach (GroupsMonster group in DataObject.MonsterList.groupsList) {
        ...
        List<MonsterInGroupTreatment> monsterInGroupTreatments =
            monsterEditor.GetTreatmentForMonsterInGroup(
                group.monstersInGroupList, originalGroup.monstersInGroupList);

        if (!Utils.Tools.IsSimilar(group, originalGroup) &&
            monsterInGroupTreatments.Count > 0) {
  ...
  private string GetSpritePath(Texture2D sprite) {
    ...
    if (spritePath.Contains(resourcesFolder)) {
      int indexOfResources =
          spritePath.IndexOf(resourcesFolder, StringComparison.CurrentCulture);
      spritePath =
          spritePath.Substring(indexOfResources + resourcesFolder.Length);

      int indexOfExtension =
          spritePath.IndexOf(".", StringComparison.CurrentCulture);
  ...
  public void RequestSaveWeapon(Weapon weapon, bool isNew) {
    ...
    form.AddField("onDamageDealt",
                  weapon.OnDamageDealt != null ? weapon.OnDamageDealt.ToString()
                  : "");
    form.AddField("onDamageReceive", weapon.OnDamageReceive !=
                                         null ? weapon.OnDamageReceive.ToString()
                  : "");
    form.AddField("model", weapon.modelName);
    form.AddField("equipmentType", (int) weapon.equipmentType);
    form.AddField("spritePath",
                  weapon.sprite != null ? GetSpritePath(weapon.sprite)
                  : "");
    ...
    if (isNew) {
      www = UnityWebRequest.Post(
          NetworkingController.PublicURL + "/api/v1/equipment/add", form);
    } else {
      www = UnityWebRequest.Post(
          NetworkingController.PublicURL + "/api/v1/equipment/update", form);
    }
  ...
  public void RequestSaveArmor(Armor armor, bool isNew) {
    ...
    form.AddField("onDamageDealt",
                  armor.OnDamageDealt != null ? armor.OnDamageDealt.ToString()
                  : "");
    form.AddField("onDamageReceive", armor.OnDamageReceive !=
                                         null ? armor.OnDamageReceive.ToString()
                  : "");
    form.AddField("model", armor.modelName);
    form.AddField("equipmentType", (int) armor.equipmentType);
    form.AddField("spritePath",
                  armor.sprite != null ? GetSpritePath(armor.sprite)
                  : "");
    form.AddField("gameToken", NetworkingController.GameToken);
    // Use for weapon
  ...
  public void RequestSaveMonster(Monster monster, bool isNew) {
    ...
    form.AddField("onDamageDealt", monster.OnDamageDealt !=
                                       null ? monster.OnDamageDealt.ToString()
                  : "");
    form.AddField("onDamageReceive",
                  monster.OnDamageReceive !=
                      null ? monster.OnDamageReceive.ToString()
                  : "");
    ...
    form.AddField("spritePath",
                  monster.sprite != null ? GetSpritePath(monster.sprite)
                  : "");
    form.AddField("gameToken", NetworkingController.GameToken);
    ...
    if (isNew) {
      www = UnityWebRequest.Post(
          NetworkingController.PublicURL + "/api/v1/monster/add", form);
    } else {
      www = UnityWebRequest.Post(
          NetworkingController.PublicURL + "/api/v1/monster/update", form);
    }
  ...
  public void RequestSaveGroupMonster(
      GroupsMonster group, bool isNew,
      List<MonsterInGroupTreatment> monsterInGroupTreatmentsList) {
    WWWForm form = new WWWForm();
    ...
    form.AddField("spritePath",
                  group.sprite != null ? GetSpritePath(group.sprite)
                  : "");
    form.AddField("gameToken", NetworkingController.GameToken);

    if (isNew) {
      foreach (MonstersInGroup monstersInGroup in group.monstersInGroupList) {
        form.AddField("monster_groups_list_monsters[]",
                      monstersInGroup.GetMonsterId());
        form.AddField("monster_groups_list_nbMonster[]",
                      monstersInGroup.nbMonster);
      }
    } else if (monsterInGroupTreatmentsList != null) {
      foreach (MonsterInGroupTreatment monsterInGroupTreatments in
                   monsterInGroupTreatmentsList) {
        form.AddField("monster_groups_list_monsters[]",
                      monsterInGroupTreatments.monstersInGroup.GetMonsterId());
        form.AddField("monster_groups_list_nbMonster[]",
                      monsterInGroupTreatments.monstersInGroup.nbMonster);
        form.AddField("monster_groups_list_treatment[]",
                      (int) monsterInGroupTreatments.treatment);
      }
    }
    ...
    if (isNew) {
      www = UnityWebRequest.Post(
          NetworkingController.PublicURL + "/api/v1/group/monster/add", form);
    } else {
      www = UnityWebRequest.Post(NetworkingController.PublicURL +
                                     "/api/v1/group/monster/update",
                                 form);
    }
  ...
  public void RequestLoadEquipment() { this.StartCoroutine(LoadEquipment()); }
  ...
  public IEnumerator LoadEquipment() {
    ...
    if (www.responseCode == 200) {
      DataObject.EquipmentList =
          new EquipmentList(null, www.downloadHandler.text);
      weaponEditor.CloneWeaponDictionary();
      armorEditor.CloneArmorDictionary();
    } else {
  ...
  public void RequestLoadMonster() { this.StartCoroutine(LoadMonster()); }
