...
namespace Games.Global.Spells.SpellsController {
  ...
  public enum OriginalDirection { None, Forward, Random }

  public class SpellController : MonoBehaviour {
    [SerializeField]
    private BuffController buffController;
    [SerializeField]
    private AreaOfEffectController areaOfEffectController;
    [SerializeField]
    ...
    public static bool CastSpell(Entity entity, Spell spell,
                                 Vector3 startPosition, Entity target = null,
                                 int spellSlotIfPlayer = 0) {
      if (spell == null && spell.cost <= entity.ressource1 - spell.cost) {
        return false;
      }

      if ((!spell.isOnCooldown && spell.nbUse != 0) ||
          (spell.activeSpellComponent != null &&
           spell.activeSpellComponent.isBasicAttack &&
           entity.weapons.Count > 0)) {
        if (spell.nbUse > 0) {
          spell.nbUse--;
        }

        spell.isOnCooldown = true;
        instance.StartCoroutine(PlayCastTime(entity, spell, startPosition,
                                             target, spellSlotIfPlayer));
        ...
        instance.StartCoroutine(
            PlayCastTime(entity, spell, startPosition, target));
      } else {
        return false;
      }
    ...
    private static void SetOriginalPosition(SpellComponent spellComponent,
      ...
      case OriginalPosition.Caster:
        ...
        if (spellComponent.needPositionToMidToEntity) {
          spellComponent.startPosition +=
              Vector3.up * (caster.entityPrefab.transform.localScale.y / 2);
        }
        break;
      case OriginalPosition.Target:
        if (target != null) {
          ...
          if (spellComponent.needPositionToMidToEntity) {
            spellComponent.startPosition +=
                Vector3.up * (target.entityPrefab.transform.localScale.y / 2);
          }
        } else {
      ...
      case OriginalDirection.Forward:
        spellComponent.initialRotation =
            caster.entityPrefab.transform.localEulerAngles;
        spellComponent.trajectoryNormalized =
            caster.entityPrefab.transform.forward;
        break;
    ...
    public static void CastSpellComponent(Entity entity,
                                          SpellComponent spellComponent,
                                          Vector3 startPosition,
                                          Entity target = null,
                                          SpellComponent originSpell = null) {
    ...
    public static IEnumerator PlayCastTime(Entity entity, Spell spell,
      ...
      while (duration > 0) {
        ...
        if (spell.wantToCastDuringCast) {
          ...
          CastSpellComponent(entity, spell.duringCastSpellComponent,
      ...
      CastSpellComponent(entity, spell.activeSpellComponent, startPosition,
                         target);
    }
    ...
    public static void CastPassiveSpell(Entity entity) {
      ...
      foreach (Spell spell in entity.spells) {
        if (spell.passiveSpellComponent != null) {
          CastSpellComponent(entity, spell.passiveSpellComponent,
                             entity.entityPrefab.positionPointed,
                             entity.entityPrefab.target);
        }
      }
    }
