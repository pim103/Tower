...
namespace Games.Global {
  public class EntityPrefab : MonoBehaviour {
    ...
    [SerializeField]
    private Rigidbody rigidbodyEntity;

    [SerializeField]
    protected MeshRenderer meshRenderer;
    [SerializeField]
    public MeshRenderer[] mrShaderTargets;
    [SerializeField]
    public SkinnedMeshRenderer[] smrShaderTargets;
    ...
    [SerializeField]
    public Transform positionInRightHand;
    [SerializeField]
    public Transform angleWithOneRight;

    [SerializeField]
    public Transform positionInLeftHand;
    [SerializeField]
    public Transform angleWithOneLeft;
    [SerializeField]
    public NavMeshAgent navMeshAgent;

    [SerializeField]
    public GameObject headCurve;
    [SerializeField]
    public GameObject thornSphere;
    [SerializeField]
    ...
    public void AddItemInHand(Weapon weapon) {
      ...
      GameObject weaponGameObject =
          Instantiate(weapon.model, hand.transform, true);
    ...
    public bool CastSpell() {
      ...
      case AttackBehaviorType.AllSpellsIFirst:
        foreach (Spell spell in entity.spells) {
          if (SpellController.CastSpell(entity, spell, transform.position,
                                        target)) {
            return true;
          }
        }
    ...
    public void FindTarget() {
      ...
      case TypeEntity.MOB:
        foreach (KeyValuePair<int, PlayerPrefab> value in DataObject
                     .playerInScene) {
          ...
          if (!player.entity.isInvisible && !player.entity.isUntargeatable &&
              !player.entity.hasNoAggro) {
            if ((dist > newDist)) {
              dist = newDist;
        ...
        foreach (Entity summon in DataObject.invocationsInScene) {
          float newDist = Vector3.Distance(
              transform.position, summon.entityPrefab.transform.position);
          if (newDist > minDistAggro) {
            continue;
          }

          if (!summon.isInvisible && !summon.isUntargeatable &&
              !summon.hasNoAggro && !findEnemyWithTaunt) {
            if ((dist > newDist)) {
              dist = newDist;
              newTarget = summon;
            }
      ...
      case TypeEntity.ALLIES:
        foreach (Monster monster in DataObject.monsterInScene) {
          ...
          float newDist = Vector3.Distance(
              transform.position, monster.entityPrefab.transform.position);
          if (newDist > minDistAggro) {
            continue;
          }

          if (!monster.isInvisible && !monster.isUntargeatable &&
              !monster.hasNoAggro) {
            if ((dist > newDist)) {
              dist = newDist;
              newTarget = monster;
