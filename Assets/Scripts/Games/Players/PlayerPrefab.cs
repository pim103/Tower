...
namespace Games.Players {
  public class PlayerPrefab : PlayerIntent, EffectInterface {
    ...
    [SerializeField]
    private LayerMask interactWithCamera;

    [SerializeField]
    private Slider hpBar;
    [SerializeField]
    private Slider ressourcesBar;
    ...
    // Use for some spell
    [SerializeField]
    private bool isFakePlayer = false;
    ...
    private void FixedUpdate() {
      ...
      if (Physics.Raycast(playerTransform.position, (camera.transform.up * -1),
    ...
    public void GetIntentPlayer() {
      if (entity.underEffects.ContainsKey(TypeEffect.Stun) ||
          entity.underEffects.ContainsKey(TypeEffect.Sleep)) {
        wantToGoBack = false;
        wantToGoForward = false;
        wantToGoLeft = false;
        wantToGoRight = false;
      ...
      animator.SetBool("isWalking", wantToGoBack | wantToGoForward |
    ...
    private void CheckPassiveSpell(Spell spell) {
      ...
      if (spell.typeSpell == TypeSpell.Passive ||
          spell.typeSpell == TypeSpell.ActiveWithPassive) {
        foreach(SpellInstruction spellInstruction in spell.spellInstructions) {
          if ((spell.typeSpell == TypeSpell.ActiveWithPassive &&
              ...
              spell.typeSpell == TypeSpell.Passive) {
            ...
            case TypeSpellInstruction.EffectOnTargetWhenDamageDeal:
              entity.damageDealExtraEffect.Add(
                  spellInstruction.effect.typeEffect, spellInstruction.effect);
              break;
            case TypeSpellInstruction.SelfEffectOnDamageReceive:
              entity.damageReceiveExtraEffect.Add(
                  spellInstruction.effect.typeEffect, spellInstruction.effect);
              break;
              ;
            case TypeSpellInstruction.ChangeBasicAttack:
              Debug.Log("Change weapon");
              entity.weapons[0].idPoolProjectile =
                  spellInstruction.idPoolObject;
              entity.weapons[0].type =
                  spellInstruction.weaponNewStats.typeWeapon;
              entity.weapons[0].damage *=
                  spellInstruction.weaponNewStats.damageModifier;
              entity.weapons[0].attSpeed *=
                  spellInstruction.weaponNewStats.attSpeedModifier;
              break;
            }
          }
        }
    ...
    private void ActivePreviewSpell(Spell spell) {
      foreach(SpellInstruction spellInstruction in spell.spellInstructions) {
        if (spellInstruction.TypeSpellInstruction ==
            TypeSpellInstruction.InstantiateSomething) {
          if (spellInstruction.typeSpellObject == TypeSpellObject.GroundArea) {
            GameObject areaSpell = spell.spellInstantiate;
            if (areaSpell == null) {
              areaSpell = ObjectPooler.SharedInstance.GetPooledObject(
                  spellInstruction.idPoolObject);
    ...
    private IEnumerator CastSpell(Spell spell) {
      ...
      foreach(SpellInstruction spellInstruction in spell.spellInstructions) {
        ...
        case TypeSpellInstruction.SelfEffect:
          ...
          break;
        case TypeSpellInstruction.EffectOnTargetWhenDamageDeal:
          StartCoroutine(AddDamageDealExtraEffect(
              spellInstruction.effect, spellInstruction.durationInstruction));
          break;
        case TypeSpellInstruction.SelfEffectOnDamageReceive:
          StartCoroutine(AddDamageReceiveExtraEffect(
              spellInstruction.effect, spellInstruction.durationInstruction));
          break;
        case TypeSpellInstruction.SpecialMovement:
          PlaySpecialMovement(spellInstruction.specialMovement,
                              spellInstruction.durationInstruction);
          break;
        }
      }
    }

    private void ActiveSpellObject(Spell spell,
                                   SpellInstruction spellInstruction) {
      ...
      case TypeSpellObject.Projectile:
        ...
        projectileSpell.transform.eulerAngles =
            camera.transform.eulerAngles + (Vector3.right * rotX);
        projectileSpell.transform.forward *= 1.5f;

        projectileSpell.SetActive(true);

        ProjectilesPrefab projectilesPrefab =
            projectileSpell.GetComponent<ProjectilesPrefab>();
        projectilesPrefab.rigidbody.AddForce(transform.forward * 1000,
      ...
      case TypeSpellObject.OnHimself:
        GameObject objectPooled = ObjectPooler.SharedInstance.GetPooledObject(
            spellInstruction.idPoolObject);
    ...
    private void CameraRotation() {
      ...
      //            virtualHand.transform.eulerAngles =
      ...
      if (Physics.Raycast(ray, out hit, 1000,
                          ~LayerMask.GetMask("Player", "Explosion"))) {
      ...
      if (Physics.Raycast(playerTransform.position,
                          (camera.transform.forward * -1), out hit, 6,
                          interactWithCamera)) {
