...
namespace Games.Transitions {
  public class TransitionDefenseAttack : MonoBehaviour {
    ...
    [SerializeField]
    private GameObject waitingOtherPlayerPanel;
    [SerializeField]
    ...
    private IEnumerator WaitingEndDefense() {
      ...
      while (defenseTimer > 0 && !hasValidated) {
        ...
        if (defenseTimer <= 10 &&
            !hoverDetector.defenseUiController.keyAlreadyPut) {
          hoverDetector.defenseUiController.keyButton.transform
              .GetComponent<Image>()
              .color = new Color(1, defenseTimer % 2, defenseTimer % 2, 1);
        }
      ...
      if (hoverDetector.currentlyBlocked) {
        if (hoverDetector.lastTileWithContent) {
          ...
          hoverDetector.lastTileWithContent.contentType =
        ...
        if (hoverDetector.defenseUiController.keyAlreadyPut) {
          ...
          hoverDetector.tileHoldingKeyGroup.contentType =
              GridTileController.TypeData.Empty;
        }
      }
    ...
    public void StartDefenseCounter() { StartCoroutine(WaitingEndDefense()); }
    ...
    private void SendGridData() {
      ...
      foreach (var gridCell in initDefense.gridCellList) {
        GridTileController cellController =
            gridCell.GetComponent<GridTileController>();
        if (cellController.contentType != GridTileController.TypeData.Empty) {
          stringToSend += cellController.coordinates.x + ":" +
                          cellController.coordinates.y + ":";
          switch (cellController.contentType) {
          case GridTileController.TypeData.Group:
            CardBehaviorInGame currentCardBehaviorInGame =
                cellController.content.GetComponent<CardBehaviorInGame>();
            stringToSend += "1:" + currentCardBehaviorInGame.groupId + ":[";
            if (currentCardBehaviorInGame.meleeWeaponSlot) {
              stringToSend += currentCardBehaviorInGame.meleeWeaponSlot
            ...
            if (currentCardBehaviorInGame.rangedWeaponSlot) {
              stringToSend += currentCardBehaviorInGame.rangedWeaponSlot
                                  .GetComponent<CardBehaviorInGame>()
                                  .equipement.id +
                              ",";
            } else {
            ...
            if (currentCardBehaviorInGame.helmetSlot) {
              stringToSend += currentCardBehaviorInGame.helmetSlot
                                  .GetComponent<CardBehaviorInGame>()
                                  .equipement.id +
                              ",";
            } else {
            ...
            if (currentCardBehaviorInGame.chestSlot) {
              stringToSend += currentCardBehaviorInGame.chestSlot
                                  .GetComponent<CardBehaviorInGame>()
                                  .equipement.id +
                              ",";
            } else {
            ...
            if (currentCardBehaviorInGame.grievesSlot) {
              stringToSend += currentCardBehaviorInGame.grievesSlot
                                  .GetComponent<CardBehaviorInGame>()
                                  .equipement.id +
                              ",";
            } else {
            ...
            /*stringToSend +=
            currentCardBehavior.rangedWeaponSlot.GetComponent<CardBehavior>().equipement.id
            + ","; stringToSend +=
            currentCardBehavior.helmetSlot.GetComponent<CardBehavior>().equipement.id
            + ","; stringToSend +=
            currentCardBehavior.chestSlot.GetComponent<CardBehavior>().equipement.id
            + ","; stringToSend +=
            currentCardBehavior.grievesSlot.GetComponent<CardBehavior>().equipement.id
            ;*/
          ...
          case GridTileController.TypeData.Trap:
            TrapBehavior currentTrapBehavior =
                cellController.content.GetComponent<TrapBehavior>();
      ...
      TowersWebSocket.TowerSender("OTHERS",
                                  NetworkingController.CurrentRoomToken, "GRID",
                                  stringToSend);
    }
  }
}
