...
namespace Games.Transitions {
  public class ChooseDeckAndClass : MonoBehaviour {
    [SerializeField]
    private Button[] buttonsOfChoice;

    [SerializeField]
    private Button validateChoices;

    [SerializeField]
    private Button[] monsterDeckButtons;
    [SerializeField]
    private Button[] equipmentDeckButtons;
    ...
    private void LaunchGame() {
      ...
      TowersWebSocket.TowerSender("SELF", NetworkingController.CurrentRoomToken,
                                  "Player", "SendClassWeapon",
                                  TowersWebSocket.FromDictToString(argsDict));
      isValidate = true;
    }
    ...
    private void GetIdentityOfButton(Button button) {
      ...
      if (identity.identityType == IdentityType.Role) {
        if (currentRoleImage != null) {
          currentRoleImage.color =
              new Color(greyColor[0], greyColor[1], greyColor[2]);
        }

        buttonImage.color =
            new Color(greenColor[0], greenColor[1], greenColor[2]);
      ...
      foreach (DeckJsonObject deckJson in dJsonObjects) {
        ...
        if (loadedDeck.type == Deck.Decktype.Monsters) {
          GameObject buttonGameObject =
              monsterDeckButtons[activeMonsterButtonCount].gameObject;
          buttonGameObject.SetActive(true);
          buttonGameObject.transform.GetChild(0).GetComponent<Text>().text =
              loadedDeck.name;
          monsterDeckButtons[activeMonsterButtonCount].onClick.AddListener(
              delegate { monsterDeckId = loadedDeck.id; });
          activeMonsterButtonCount++;
        }

        if (loadedDeck.type == Deck.Decktype.Equipments) {
          GameObject buttonGameObject =
              equipmentDeckButtons[activeEquipmentButtonCount].gameObject;
          buttonGameObject.SetActive(true);
          buttonGameObject.transform.GetChild(0).GetComponent<Text>().text =
              loadedDeck.name;
          equipmentDeckButtons[activeEquipmentButtonCount].onClick.AddListener(
              delegate { equipmentDeckId = loadedDeck.id; });
