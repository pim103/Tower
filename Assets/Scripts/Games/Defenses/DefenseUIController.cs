...
namespace Games.Defenses {
  public class DefenseUIController : MonoBehaviour {
    ...
    public void PutCardBackToHand(GameObject equipementCard) {
      if (equipementCard) {
        CardBehaviorInGame equipementCardBehaviorInGame =
            equipementCard.GetComponent<CardBehaviorInGame>();
        equipementCardBehaviorInGame.groupParent.SetActive(false);
        equipementCardBehaviorInGame.groupParent.transform.localPosition =
            Vector3.zero;
        equipementCardBehaviorInGame.ownMeshRenderer.enabled = true;
        equipementCardBehaviorInGame.transform.SetParent(
            equipementCardBehaviorInGame.ownCardContainer);
        equipementCardBehaviorInGame.transform.localPosition = Vector3.zero;
        equipementCardBehaviorInGame.gameObject.layer =
            LayerMask.NameToLayer("Card");
        equipementCard.SetActive(true);
        hoverDetector.currentResource +=
            equipementCardBehaviorInGame.equipement.cost;
    ...
    public void DrawCards() {
      foreach (var cardContainer in mobCardContainers) {
        if (cardContainer.transform.childCount == 0 &&
            cardsInMonsterDeck.Count > 0) {
          Card selectedCard =
              cardsInMonsterDeck[Random.Range(0, cardsInMonsterDeck.Count - 1)];
      ...
      foreach (var cardContainer in equipementCardContainers) {
        if (cardContainer.transform.childCount == 0 &&
            cardsInEquipmentDeck.Count > 0) {
          Card selectedCard = cardsInEquipmentDeck[Random.Range(
              0, cardsInEquipmentDeck.Count - 1)];
