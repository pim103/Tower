  ...
  public class InitDefense : MonoBehaviour {
    ...
    public void Init() {
      ...
      if (currentLevel < maps.Length) {
        currentMap = maps[currentLevel].mapsInLevel[(
            NetworkingController.CurrentRoomMapsLevel !=
            null ? NetworkingController.CurrentRoomMapsLevel[currentLevel] : 0)];
        ...
        if (!se.gameController.byPassDefense) {
          Generate();
          defenseCamera.transform.position =
              currentMapStats.cameraPosition.transform.position;
    ...
    private void Generate() {
      ...
      for (int i = currentMapStats.mapWidth * -1; i < currentMapStats.mapWidth;
           i += 2) {
        for (int j = currentMapStats.mapHeight * -1;
             j < currentMapStats.mapHeight; j += 2) {
          currentCell = Instantiate(
              gridCell,
              new Vector3(i + currentMap.transform.localPosition.x + 1, 3f,
                          j + currentMap.transform.localPosition.z + 1),
              Quaternion.Euler(90, 0, 0));
          currentCell.transform.parent = currentMap.transform;
          GridTileController currentTileController =
              currentCell.GetComponent<GridTileController>();
