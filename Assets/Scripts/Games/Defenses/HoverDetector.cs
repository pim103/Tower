    ...
    void Update() {
      ...
      if (Physics.Raycast(ray, out hit, 500f, gridMask)) {
        ...
        currentTileController =
            hit.collider.gameObject.GetComponent<GridTileController>();
        // Debug.Log(objectInHand);
        // Debug.Log(currentCardBehavior.cardType);
        if (objectInHand &&
            (objectInHand.layer == LayerMask.NameToLayer("CardInHand") &&
                 currentCardBehaviorInGame.cardType == 1 ||
             objectInHand.layer == LayerMask.NameToLayer("Key"))) {
          if (currentTileController.contentType ==
              GridTileController.TypeData.Group) {
        ...
        } else if (currentTileController.contentType !=
                       GridTileController.TypeData.Empty ||
                   pathToEnd.status != NavMeshPathStatus.PathComplete ||
                   currentTileController.isTooCloseFromAMob ||
                   (objectInHand &&
                    objectInHand.layer == LayerMask.NameToLayer("CardInHand") &&
                    currentCardBehaviorInGame.cardType == 0 &&
                    !currentCardBehaviorInGame.groupRangeBehavior
                         .CheckContentEmpty())) {
          currentTileController.ChangeColorToRed();
          canPutItHere = false;
        } else {
        ...
        if (objectInHand) {
          if (objectInHand.layer == LayerMask.NameToLayer("Wall") ||
              objectInHand.layer == LayerMask.NameToLayer("Trap")) {
            objectInHand.transform.position =
                hit.collider.gameObject.transform.position +
                Vector3.down * 2.25f;
          } else if (objectInHand.layer ==
                     LayerMask.NameToLayer("CardInHand")) {
            objectInHand.transform.position =
                hit.collider.gameObject.transform.position +
                Vector3.down * 1.5f;
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0) || puttingWalls) {
          if (canPutItHere && objectInHand) {
            ...
            NavMesh.CalculatePath(startPos.transform.position,
                                  dest.transform.position, NavMesh.AllAreas,
                                  pathToEnd);
            if (defenseUiController.keyAlreadyPut) {
              pathToKey = new NavMeshPath();
              NavMesh.CalculatePath(
                  startPos.transform.position,
                  defenseUiController.keyObject.transform.position,
                  NavMesh.AllAreas, pathToKey);
            }
            if (pathToEnd.status == NavMeshPathStatus.PathComplete &&
                (pathToKey.status == NavMeshPathStatus.PathComplete ||
                 !defenseUiController.keyAlreadyPut)) {
              currentlyBlocked = false;
              if (objectInHand.layer == LayerMask.NameToLayer("Wall") ||
                  (objectInHand.layer == LayerMask.NameToLayer("CardInHand") && currentCardBehaviorInGame
                                                                                        .cardType !=
                                                                                    1 /*&& currentCardBehaviorInGame.group.cost <= currentResource*/) ||
                  objectInHand.layer == LayerMask.NameToLayer("Trap") &&
                      currentResource >= 1) {
                ...
                if (objectInHand.layer == LayerMask.NameToLayer("Wall")) {
                  currentTileController.contentType =
                      GridTileController.TypeData.Wall;
                  puttingWalls = true;
                } else if (objectInHand.layer ==
                           LayerMask.NameToLayer("CardInHand")) {
                  currentTileController.contentType =
                      GridTileController.TypeData.Group;
                  currentCardBehaviorInGame.groupRangeBehavior.SetAllTilesTo(
                      true);
                  // currentResource -= currentCardBehaviorInGame.group.cost;
                  // currentCardBehavior.groupRange.SetActive(true);
                } else if (objectInHand.layer ==
                           LayerMask.NameToLayer("Trap")) {
                  currentTileController.contentType =
                      GridTileController.TypeData.Trap;
                  // currentResource--;
                }

                objectInHand = null;
                if (lastObjectPutInPlay.layer ==
                    LayerMask.NameToLayer("Wall")) {
                  defenseUiController.PutWallInHand();
                }

                defenseUiController.currentResourceText.text =
                    currentResource.ToString();
              } else if (
                  objectInHand.layer == LayerMask.NameToLayer("CardInHand") && currentCardBehaviorInGame
                                                                                       .cardType ==
                                                                                   1 /*&& currentCardBehaviorInGame.equipement.cost <= currentResource*/) {
                lastObjectPutInPlay = objectInHand;
                lastTileWithContent = currentTileController;
                CardBehaviorInGame contentCardBehaviorInGame =
                    currentTileController.content
                        .GetComponent<CardBehaviorInGame>();
                switch (currentCardBehaviorInGame.equipement.type) {
                case TypeWeapon.Distance:
                  if (contentCardBehaviorInGame.rangedWeaponSlot) {
                    defenseUiController.PutCardBackToHand(
                        contentCardBehaviorInGame.rangedWeaponSlot);
                  }
                ...
                case TypeWeapon.Cac:
                  if (contentCardBehaviorInGame.meleeWeaponSlot) {
                    defenseUiController.PutCardBackToHand(
                        contentCardBehaviorInGame.meleeWeaponSlot);
                  }
            ...
            if (objectInHand.layer == LayerMask.NameToLayer("CardInHand")) {
              currentCardBehaviorInGame =
                  objectInHand.GetComponent<CardBehaviorInGame>();
        ...
        if (Input.GetKeyDown(KeyCode.Mouse1)) {
          if (!canPutItHere &&
              currentTileController.contentType !=
                  GridTileController.TypeData.Empty &&
              currentTileController.content.layer ==
                  LayerMask.NameToLayer("Wall") &&
              !objectInHand) {
            ...
            defenseUiController.wallButtonText.text =
          ...
          } /*else if (objectInHand)
              {
                  if (objectInHand.layer == LayerMask.NameToLayer("Wall"))
                  {
                      ...
                      objectInHand = null;
                      defenseUiController.currentWallNumber += 1;
                      defenseUiController.wallButtonText.text = "Mur x" +
              defenseUiController.currentWallNumber; } else if
              (objectInHand.layer == LayerMask.NameToLayer("Trap"))
                  {
                      objectInHand.SetActive(false);
                      objectInHand = null;
      ...
      if (Physics.Raycast(ray, out hit, 100f, cardMask)) {
        if (Input.GetKeyDown(KeyCode.Mouse0)) {
          CardBehaviorInGame hitCardBehavior =
              hit.transform.GetComponent<CardBehaviorInGame>();
          if (hitCardBehavior.cardType == 0 &&
                  hitCardBehavior.group.cost <= currentResource ||
              hitCardBehavior.cardType == 1 &&
                  hitCardBehavior.equipement.cost <= currentResource) {
            if (objectInHand) {
              if (objectInHand.layer == LayerMask.NameToLayer("CardInHand")) {
      ...
      if (Input.GetKeyDown(KeyCode.Mouse1) && objectInHand) {
        ...
        if (objectInHand.layer == LayerMask.NameToLayer("CardInHand")) {
          defenseUiController.PutCardBackToHand(
              currentCardBehaviorInGame.meleeWeaponSlot);
          defenseUiController.PutCardBackToHand(
              currentCardBehaviorInGame.rangedWeaponSlot);
          ...
          currentCardBehaviorInGame.groupParent.transform.localPosition =
              Vector3.zero;
          ...
          objectInHand.transform.SetParent(
              currentCardBehaviorInGame.ownCardContainer);
      ...
      if (Input.GetAxis("Mouse ScrollWheel") > 0f && objectInHand &&
          objectInHand.layer == LayerMask.NameToLayer("Trap")) {
        objectInHand.GetComponent<TrapBehavior>().rotation += 1;
        objectInHand.transform.Rotate(Vector3.up, 90);
      } else if (Input.GetAxis("Mouse ScrollWheel") < 0f && objectInHand &&
                 objectInHand.layer == LayerMask.NameToLayer("Trap")) {
        objectInHand.GetComponent<TrapBehavior>().rotation -= 1;
        objectInHand.transform.Rotate(Vector3.up, -90);
      }
      ...
      if (pathToEnd.status != NavMeshPathStatus.PathComplete ||
        ...
        if (lastTileWithContent) {
          ...
          WarningPanelText.text =
              "Chemin bloqué : Bougez le bloc ou cliquez droit pour annuler";
        }
      }
    }
  }
