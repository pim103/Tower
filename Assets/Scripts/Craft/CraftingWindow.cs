...
/// <summary>
/// This is the CraftingWindow script, it contains functionality that is
/// specific to the craft window
/// </summary>
public class CraftingWindow : MonoBehaviour {
  ...
  private void Start() {
    ...
    foreach(CraftingRecipeUI craftingRecipeUI in craftingRecipeUIs) {
      craftingRecipeUI.ResourceOnPointerEnterEvent +=
          slot => { ResourceOnPointerEnterEvent?.Invoke(slot); };
      craftingRecipeUI.ResourceOnPointerExitEvent +=
          slot => { ResourceOnPointerExitEvent?.Invoke(slot); };

      craftingRecipeUI.OnPointerEnterEvent +=
          slot => { OnPointerEnterEvent?.Invoke(slot); };
      craftingRecipeUI.OnPointerExitEvent +=
          slot => { OnPointerExitEvent?.Invoke(slot); };
    }
  }

  private void Init() {
    recipeUIParent.GetComponentsInChildren<CraftingRecipeUI>(
        includeInactive
        : true, result
        : craftingRecipeUIs);
  ...
  public void UpdateCraftingRecipes() {
    for (int i = 0; i < CraftingRecipes.Count; i++) {
      if (craftingRecipeUIs.Count == i) {
        craftingRecipeUIs.Add(
            Instantiate(recipeUIPrefab, recipeUIParent, false));
